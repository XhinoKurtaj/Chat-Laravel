            ...
            <div class="col-md-10 col-md-offset-1">
                <div class="row">
                    <div class="col-3">
                        ...
                        <a onclick="return confirm('Are you sure u want to delete this conversation?')" href="{{ route('conversation.delete', $conversationDetails[0]->id) }}"  class="btn btn-outline-danger btn-sm position" >
                            <i class="far fa-trash-alt"> Delete Conversation</i></a>
                    </div>
                    <div class="col-4">
                        <h5><i class="far fa-edit"></i> Update Conversation</h5><hr>
                        <form enctype="multipart/form-data" action="{{route('show.conversation',request()->route('id'))}}" method="POST">
                            ...
                            @if($conversationDetails[0]->type == 'group')
                                ...
                                <input type="text" name="custom_name" class="form-control" id="InputName"  placeholder="Enter Conversation Name" value="{{$conversationDetails[0]->custom_name}}">
                                </div>
                            @else
                                <input type="hidden" name="custom_name" placeholder="Conversation name" value="{{$conversationDetails[0]->custom_name}}">
                            @endif
                            <div class="form-group">
                        ...
                        <div id="message-list">
                            @foreach($conversationDetails[0]->message as $message)
                                <div class="alert alert-primary" role="alert">
                                    <a class="style-link" href='/admin/users/{{$message->sender->id}}'> {{$message->sender->fullName}}</a> :
                                    <a  href='/admin/messages/{{$message->id}}' class="style-link"> {{$message->message}}</a>
                            ...
                            @if(strpos($mime, '.jpg')|| strpos($mime, '.jpeg') ||strpos($mime, '.png') ||strpos($mime, '.gif'))
                                ...
                                @else
                                        <div class='column ahref-style'>
                                            <a style='text-decoration:none' href='/home/conversation/{{$conversationDetails[0]->id}}/download/{{$attachment->id}}'>
                                                {{$attachment->attachment}}<br><br><i class='fas fa-download' style='color:gray'></i>
                                            </a>
                                        </div>
                                @endif
                        @endforeach
